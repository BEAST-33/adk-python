...
class StorageSession(Base):
  ...
  def _dialect_name(self) -> str:
    ...
    return session.bind.dialect.name if session else ""

  @property
  def create_timestamp_utc(self) -> datetime:
    return _convert_datetime_to_utc(self.create_time, self._dialect_name)

  @property
  def update_timestamp_utc(self) -> datetime:
    return _convert_datetime_to_utc(self.update_time, self._dialect_name)
    ...
    with self.database_session_factory() as session_factory:
      ...
      if storage_session.update_timestamp_utc > session.last_update_time:
        raise ValueError(
            ...
            f" {datetime.fromtimestamp(storage_session.update_timestamp_utc):'%Y-%m-%d %H:%M:%S'}."
...
def _convert_datetime_to_utc(dt: DateTime, dialect_name: str) -> datetime:
  if dialect_name == "sqlite":
    ...
    # For other dialects, SQLAlchemy returns a datetime object with timezone
    # information. We can safely convert it to UTC.
