...
def route_search(query: str, top_k: int = 5) -> RouteSearchResponse:
    """
    ...
    try:
        ...
        try:
            ...
            for row in rows:
                ...
                # Apply filtering if source/destination were extracted
                if source and hasattr(row, 'sourcename') and row.sourcename != source:
                    continue
                if destination and hasattr(row, 'destinationname') and row.destinationname != destination:
                    continue
                
                route_info = {
                    "route_id": getattr(row, 'routeid', 'N/A'),
                    ...
                    "similarity_score": getattr(row, 'cosine_similarity', 0.0)
                }
...
Present the results in a organized way, highlighting key information like:
...
If no routes are found, suggest alternative approaches or ask for more specific information.
    """,
    output_key="routes_data"
)

# --- Example Usage ---
if __name__ == "__main__":
    print("Route Agent initialized successfully!")
    print("You can now interact with the agent to search for bus routes.")
    ...
    async def test_queries():
        for query in example_queries:
            print(f"\n--- Query: {query} ---")
            try:
                input_data = RouteAgentInput(query=query)
                output_data = RouteAgentOutput()
                
                print("Sending query to Route Agent...")
                async for response in Route_agent.run_async(input_data):
                    print(f"Received response: {response}")
                    if isinstance(response, dict) and 'routes_data' in response:
                        output_data.routes_data = response['routes_data']
                        print("Updated routes_data")
                
                print(f"Final Output: {output_data.model_dump()}")
            except Exception as e:
                import traceback
                print(f"Error: {e}")
