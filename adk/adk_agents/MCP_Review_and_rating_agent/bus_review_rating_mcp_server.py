...
# Instantiate the ADK tools you want to expose.
...
app = Server("bus-review-rating-mcp-server")
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    ...
    # Convert both ADK tools' definitions to the MCP Tool schema format
    mcp_tools = []
...
async def call_mcp_tool(
    ...
    # Check if the requested tool name matches any of our wrapped ADK tools
    if name in adk_tools:
        try:
            # Get the corresponding ADK tool
...
if __name__ == "__main__":
    print("Launching MCP Server to expose ADK Bus Review and Rating tools via stdio...")
    try:
