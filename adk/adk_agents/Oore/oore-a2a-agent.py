...
logger = logging.getLogger("redbus-a2a-agent")

REDBUS_API_URL = "http://oore-india.redbus.com:6666/srp/"

def log_request(endpoint: str, payload):
    logger.info(f"[{endpoint}] Incoming request:\n{json.dumps(payload, indent=2)}")

def log_response(endpoint: str, response):
    logger.info(f"[{endpoint}] Outgoing response:\n{json.dumps(response, indent=2)}")

def log_exception(endpoint: str, exc: Exception):
    logger.error(f"[{endpoint}] Exception occurred: {exc}\n{traceback.format_exc()}")

def make_a2a_response(request_id, task_id, state, message, artifacts=None):
...
def parse_user_input(user_text):
    # Updated regex to match "source: 123, destination: 124, date: 27th May 2025" format
    match = re.search(
        r"source[:\s]+(\d+).*destination[:\s]+(\d+).*date[:\s]+(\d{1,2}(?:st|nd|rd|th)? [A-Za-z]+ \d{4})(?:.*chcode (\w+))?",
    ...
    if match:
        ...
        chcode = match.group(4) if match.group(4) else "WEB_DIRECT"
        doj = datetime.strptime(
            re.sub(r'(st|nd|rd|th)', '', date_str), "%d %B %Y"
        ).strftime("%Y-%m-%d")
        return source_id, dest_id, doj, chcode
    return None, None, None, "WEB_DIRECT"

def get_redbus_data(source_id, dest_id, doj, chcode="WEB_DIRECT"):
    params = {
        "slc": 1,
        "d": dest_id,
        "s": source_id,
    ...
    try:
        ...
        logger.info("RedBus API call successful.", resp)
        data = resp.json()
        result = json.dumps(data) if data else json.dumps({"message": "Data not available"})
        return result
    except Exception as e:
        logger.error(f"RedBus API error: {e}\n{traceback.format_exc()}")
        return None

def summarize_buses(bus_data):
    if not bus_data:
        return "No buses found for your request."
    lines = []
    for bus_id, bus in list(bus_data.items())[:5]:  # Show up to 5 options
        op = f"₹{min(bus['OP'])/100:.2f}" if bus.get("OP") else "N/A"
        dp = f"₹{min(bus['DP'])/100:.2f}" if bus.get("DP") else "N/A"
...
async def root(request: Request):
    ...
    try:
        ...
        if not user_text:
            response = make_a2a_response(request_id, task_id, "failed", "No input provided.")
            log_response(endpoint, response)
            return JSONResponse(response)
        source_id, dest_id, doj, chcode = parse_user_input(user_text)
        if not source_id or not dest_id or not doj:
            response = make_a2a_response(request_id, task_id, "failed", "Could not parse source, destination, or date from your input.")
            log_response(endpoint, response)
            return JSONResponse(response)
        bus_data = get_redbus_data(source_id, dest_id, doj, chcode)
        response = make_a2a_response(request_id, task_id, "completed", bus_data)
        log_response(endpoint, response)
        return JSONResponse(response)
    except Exception as exc:
        log_exception(endpoint, exc)
        response = make_a2a_response(
...
async def tasks_send(request: Request):
    ...
    try:
        payload = await request.json()
        log_request(endpoint, payload)
        # Just reuse the root handler logic for now
        response = await root(request)
        # root() already logs response
        return response
    except Exception as exc:
        log_exception(endpoint, exc)
        response = make_a2a_response(
...
async def agent_manifest(request: Request):
    ...
    try:
        logger.info(f"[{endpoint}] Manifest requested from {request.client.host} for URL {request.url}")
        manifest = {
            "name": "RedBus OORE Read Deal A2A Agent",
            "description": "Fetches read deals offered for a given source, destination, and date.",
            "url": "http://10.5.10.190:8022/",
            "version": "1.0.0",
            "provider": {
                "organization": "redBus",
                "url": "https://redbus.in"
            ...
            "skills": [
                {
                    "id": "get_redbus_deals",
                    "name": "Get RedBus Deals",
                    "description": "Returns RedBus schedules and deals for a given source and destination and date.",
                    "tags": ["bus", "redbus", "deals", "schedules"],
                    "examples": [
                        "Show me red deals for source id 123 and destination id 124 for 26th May 2025"
                    ],
                    "inputModes": ["text"],
                    "outputModes": ["text"]
                }
        ...
        log_response(endpoint, manifest)
        return JSONResponse(manifest)
    except Exception as exc:
        log_exception(endpoint, exc)
        return JSONResponse({"error": "Could not generate manifest"}, status_code=500)

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("oore-a2a-agent:app", host="10.5.10.190", port=8022, reload=True)
